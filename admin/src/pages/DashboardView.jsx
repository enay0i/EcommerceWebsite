...
const[numb,setNumb]=useState(0);
    ...
    useEffect(() => {
        axios.get('http://localhost:4000/orderList/order')
            .then(response => {
...
setNumb(data.length);
                ...
                data.forEach(invoice => {
                    ...
                    const totalPrice = invoice.totalMoney*0.10;
                ...
                const totalPriceSumMonth = data.reduce((sum, invoice) => {
                    const invoiceDate = new Date(invoice.date);
                    return sum + (invoiceDate.getMonth() === currentDate.getMonth() && invoiceDate.getFullYear() === currentYear ? invoice.totalMoney : 0);
                }, 0);

                const totalPriceSumYear = data.reduce((sum, invoice) => {
                    return sum + (new Date(invoice.date).getFullYear() === currentYear ? invoice.totalMoney : 0);
                }, 0);

                const totalPriceSum = data.reduce((sum, invoice) => {
                    return sum + invoice.totalMoney;
                }, 0);
    ...
    return (
        <div className='px-[25px] pt-[25px] bg-[#F8F9FC] pb-[40px]'>
            <div className='flex items-center justify-between'>
                ...
                <button className='bg-[#EE6B6B]  h-[32px] rounded-[3px] text-white flex items-center justify-center px-[8px] hover:bg-orange-600'>Tạo Báo Cáo</button>
            </div>
            ...
            <div className='flex mt-[22px] w-full gap-[30px]'>
                <div className='basis-[70%] border bg-white shadow-md cursor-pointer rounded-[4px]'>
                    <div className='bg-[#F8F9FC] flex items-center justify-between py-[15px] px-[20px] border-b-[1px] border-[#EDEDED] mb-[20px]'>
                        <h2 className='text-[#EE6B6B] text-[16px] leading-[19px] font-bold'>LỢI NHUẬN TỔNG QUAN</h2>
